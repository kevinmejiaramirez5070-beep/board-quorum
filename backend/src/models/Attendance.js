...
class Attendance {
  ...
  static async create(data) {
    const { 
      ...
      // MD-05 §11 / MD-09 — trazabilidad del registro manual de contingencia
      manual_motivo = null, registered_by = null,
      manual_curso = null, contingencia = false
    } = data;
    const isPostgreSQL = !!process.env.DATABASE_URL || process.env.DB_TYPE === 'postgresql';
    const pendingValue = isPostgreSQL ? (pending_approval ? 'true' : 'false') : (pending_approval ? 1 : 0);
    const contingenciaValue = isPostgreSQL ? (contingencia ? 'true' : 'false') : (contingencia ? 1 : 0);
    ...
    const [result] = await db.execute(
      `INSERT INTO attendance (meeting_id, member_id, status, arrival_time, acting_as_principal, 
        pending_approval, manual_name, manual_position, manual_document, manual_motivo, registered_by,
        manual_curso, contingencia, created_at)
       VALUES (?, ?, ?, ?, ?, ${pendingValue}, ?, ?, ?, ?, ?, ?, ${contingenciaValue}, NOW())${returningClause}`,
      [meeting_id, memberIdValue, status, arrival_time, acting_as_principal, 
       manual_name, manual_position, manual_document, manual_motivo, registered_by, manual_curso]
    );

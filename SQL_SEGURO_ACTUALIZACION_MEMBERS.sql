...
-- 9. Actualizar valores existentes (solo si las columnas existen)
SET @preparedStatement = (SELECT IF(
  (
    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE
  ...
  'UPDATE members SET tipo_participante = ''PRINCIPAL'' WHERE member_type = ''principal'' AND (tipo_participante IS NULL OR tipo_participante = ''''); UPDATE members SET tipo_participante = ''SUPLENTE'' WHERE member_type = ''suplente'' AND (tipo_participante IS NULL OR tipo_participante = ''''); UPDATE members SET tipo_participante = ''JUNTA_DE_VIGILANCIA'' WHERE member_type = ''junta_vigilancia'' AND (tipo_participante IS NULL OR tipo_participante = '''');',
...
-- 10. Actualizar cuenta_quorum y puede_votar (solo si las columnas existen)
SET @preparedStatement = (SELECT IF(
  (
    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE
